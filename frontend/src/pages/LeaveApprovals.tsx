...
interface Leave {
  ...
  start_date: string;
  end_date: string;
  reason: string; // Include all possible statuses from your backend LeaveStatus enum
  status:
    | "Pending"
    | "Approved"
    | "Rejected"
...
interface UpdateLeaveStatusSuccessResponse {
  message: string;
  leaveId: number; // This should return the FINAL status set by the backend logic
  newStatus:
    | "Pending"
    | "Approved"
    | "Rejected"
...
function LeaveApprovals() {
  ...
  const { user, token } = useAuth() as AuthContextType; // State for pending leave requests list

  const [pendingLeaves, setPendingLeaves] = useState<Leave[]>([]); // State for overall page loading/error
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null); // State for handling approval/rejection actions specifically
  ...
  const [actionSuccess, setActionSuccess] = useState<string | null>(null); // To display success messages for actions // Check if the logged-in user has the required role to view this page

  const isManagerOrAdmin =
    user &&
    (user.role_id === MANAGER_ROLE_ID || user.role_id === ADMIN_ROLE_ID); // Effect to fetch pending leave requests when the component mounts or user/token changes
  ...
  useEffect(() => {
    const fetchPendingLeaves = async () => {
      ...
      setError(null);
      try {
        let endpoint = ""; // Determine which backend endpoint to call based on the user's role
        ...
        if (endpoint) {
          console.log("Token before:", token); // Use the api helper to call the determined backend endpoint
        ...
        console.error("Error fetching pending leave requests:", err); // Display a more user-friendly error message from the backend response if available
        const errorMessage =
    ...
    if (user && token && isManagerOrAdmin) {
      ...
      if (!token) {
        ...
        // Check user exists before assuming role
  ...
  const handleProcessLeave = async (
    ...
    try {
      let endpoint = ""; // Determine which backend endpoint to call based on the logged-in user's role
      if (user.role_id === MANAGER_ROLE_ID) {
        // Managers use the /api/leaves/status/:id endpoint for their direct reports
        endpoint = `/api/leaves/status/${leaveId}`;
        console.log(
          `Processing leave ${leaveId} as Manager. Calling endpoint: ${endpoint}`
        );
      } else if (user.role_id === ADMIN_ROLE_ID) {
        ...
        console.log(
          `Processing leave ${leaveId} as Admin. Calling endpoint: ${endpoint}`
        );
      } else {
      ...
      if (!endpoint) {
        setActionError(
          "Could not determine processing endpoint for your role."
        );
        setIsSubmitting(false); // Re-enable buttons immediately on known error
        return;
      } // Call the determined backend endpoint with the status // Note: Ensure backend handlers return UpdateLeaveStatusSuccessResponse or an error structure
      ...
      console.log(
        `Leave request ${leaveId} processed with status ${status}:`,
        response
      );
      setActionSuccess(
        `Leave request ${leaveId} ${status.toLowerCase()} successfully.`
      );
      setPendingLeaves((prevLeaves) =>
        prevLeaves.filter((leave) => leave.leave_id !== leaveId)
      );
    } catch (err: any) {
      console.error(`Error processing leave request ${leaveId}:`, err); // Display a more user-friendly error message from the backend response if available
      const errorMessage =
        err.response?.data?.message ||
        err.message ||
        `Failed to process leave request ${leaveId} with status ${status}.`;
  ...
  return (
    <div className="leave-approvals-container">
      ...
      {loading && (
        <p className="loading-message">Loading pending leave requests...</p>
      )}
                  {/* Use inline style for error for quick visibility */}       
          {error && <p style={{ color: "red" }}>Error: {error}</p>}           {" "}
      {/* Display action status messages */}           {" "}
      {actionError && (
        <p className="action-error">Action Error: {actionError}</p>
      )}
                 {" "}
      {actionSuccess && (
        <p className="action-success">Action Success: {actionSuccess}</p>
      )}
        ...
        <table className="pending-requests-table">
          ...
          <thead>
            ...
            <tr>
              ...
              {/* Column for Approve/Reject buttons */}                       {" "}
            </tr>
                               {" "}
          </thead>
                             {" "}
          <tbody>
            ...
            {pendingLeaves.map((leave) => (
              <tr key={leave.leave_id}>
                ...
                       {" "}
                <td>
                  {leave.user?.name || "N/A"} ({leave.user?.email || "N/A"})
                </td>
