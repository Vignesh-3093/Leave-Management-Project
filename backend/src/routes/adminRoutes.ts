...
interface UserWithBalancesResponse extends UserResponse {
  ...
  leaveBalances: {
    // Define the structure of a balance summary for a single leave type
...
> = async (req: AuthenticatedRequest, res): Promise<void> => {
  const admin_user_id = req.user?.user_id;
  const admin_user_role_id = req.user?.role_id; // Destructure data for the new leave type from the request body

  const { name, requires_approval, is_balance_based } = req.body; // If authentication failed or user info missing (should be caught by protect, but defensive check)
  ...
  try {
    const newLeaveType = new LeaveType(); // Assign properties from the request body
    ...
    const createdLeaveType = await leaveTypeRepository.save(newLeaveType); // Log the successful creation (optional)
...
> = async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  const admin_user_id = req.user?.user_id;
  const admin_user_role_id = req.user?.role_id; // Destructure data for the new user from the request body

  const { name, email, password, role_id, manager_id = null } = req.body; // manager_id is optional, default to null // If authentication failed or user info missing (should be caught by protect, but defensive check)
  ...
  try {
    ...
    if (existingUser) {
      ...
      return; // Exit handler
    } // --- Check if Provided Role Exists --- // Query the database to ensure the provided role_id is valid
    ...
    const newUser = new User(); // Assign properties from the request body
    newUser.name = name.trim();
    newUser.email = email.trim();
    newUser.password_hash = hashedPassword; // Store the hashed password
    newUser.role = role;
    newUser.manager_id = manager_id; // Assign manager_id (can be null or a valid manager's user_id) // Save the new user entity to the database

    const createdUser = await userRepository.save(newUser); // --- IMPLEMENTATION: Trigger Initial Leave Balance Creation Here ---
    ...
    try {
      ...
      if (balancesToCreate.length > 0) {
        const currentYear = new Date().getFullYear(); // Get the current year // Fetch the LeaveType entities needed based on the names in the rules
        ...
        const leaveTypeMap = new Map(leaveTypes.map((lt) => [lt.name, lt])); // Map names to entities // Create LeaveBalance entities for the new user based on the rules and fetched LeaveTypes
...
> = async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  const admin_user_id = req.user?.user_id;
  const admin_user_role_id = req.user?.role_id; //console.log(`--- Admin user ${admin_user_id} accessing /api/admin/users ---`); // If authentication failed or user info is missing (should be caught by protect, but defensive check)
  ...
  const roleIdParam = req.query.role_id; // Convert the role_id query parameter to a number, if it exists and is a valid number
  ...
  try {
    ...
    const users = await userRepository.find(findOptions); //console.log(`Workspaceed ${users.length} users.`);
    const usersWithBalances: UserWithBalancesResponse[] = [];
    const currentYear = new Date().getFullYear(); // Get current year for balance fetch // --- Fetch Leave Balances for each user ---
    ...
    } // If no users found, usersWithBalances remains empty //console.log(`Prepared response for ${usersWithBalances.length} users including balances.`); // Send the list of users (with balances) back

    res.status(200).json(usersWithBalances);
    return; // Exit handler
  } catch (error) {
    // Catch any errors during the database query
    console.error("Error fetching users for admin:", error); // Send a 500 Internal Server Error response
...
> = async (req: AuthenticatedRequest, res): Promise<void> => {
  const admin_user_id = req.user?.user_id;
  const admin_user_role_id = req.user?.role_id; //console.log(`--- Admin user ${admin_user_id} accessing leave requests needing Admin approval ---`); // If authentication failed or user info is missing (should be handled by middleware)
  ...
  try {
    const leavesNeedingAdminApproval = await leaveRepository.find({
      where: [
        // Use an array for OR conditions
    ...
    }); //console.log(`Workspaceed ${leavesNeedingAdminApproval.length} leave requests needing Admin approval.`); // Send the list in the response
...
const updateLeaveStatusByAdminHandler: RequestHandler<
  ...
  | { message: string }, // Res Body (success or error)
...
> = async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  try {
    ...
    const deleteResult = await leaveTypeRepository.delete(leaveTypeId); // Check if a row was actually affected (means the leave type existed)
    ...
    console.error(
      ...
      error
    ); // Log the specific error if needed for debugging // If it's a database error not caught by the usage checks above, it will be a 500 error
    res
      .status(500)
...
const bulkInsertLeavesToRedis: RequestHandler = async (req, res) => {
  console.log("Attempting to bulk insert leaves to redis..");
  try {
    const redis = await getRedisClient();
    const allLeaves = await leaveRepository.find();

    if (allLeaves.length > 0) {
      const pipeline = redis.multi(); // Create a pipeline object

      for (const leave of allLeaves) {
        console.log(`Type of leave.start_date:`, typeof leave.start_date); // <-- Add this line
        const redisKey = `leave:${leave.leave_id}`;
        pipeline.hSet(redisKey, {
          ...
          processed_by_id:
            leave.processed_by_id === null ? "" : leave.processed_by_id,
          processed_at: leave.processed_at
            ? leave.processed_at.toISOString()
            : "",
        });
      }
      const results = await pipeline.exec(); // Execute the pipeline
      console.log("Bulk insert to Redis completed:", results);
      res.status(200).send({ message: "Bulk insert to Redis completed." });
    } else {
      res
        .status(200)
        .send({ message: "No leave applications found in MySQL." });
    }
  } catch (error) {
    console.error("Error performing bulk insert to Redis:", error);
    res.status(500).send({ error: "Failed to perform bulk insert to Redis." });
  }
};
